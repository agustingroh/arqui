...
public interface PersonCareerRepository extends JpaRepository<PersonCareer, PersonCareerId> {
    ...
    @Query(value = "SELECT a.career_id, a.name, a.year ,SUM(a.enrolled) as enrolled,SUM(a.graduated) as graduated FROM (SELECT pc.career_id,c.name, EXTRACT(YEAR FROM pc.init_date) as year, COUNT(*) as enrolled, '0' as graduated FROM person_career pc JOIN Career c ON c.id=pc.career_id GROUP BY pc.career_id,c.name,year UNION (SELECT pc.career_id,c.name, EXTRACT(YEAR FROM pc.due_date) AS year,'0' as enrolled ,COUNT(*) as graduated FROM person_career pc JOIN Career c ON c.id=pc.career_id GROUP BY pc.career_id,c.name,pc.due_date HAVING pc.due_date IS NOT NULL )) as a GROUP BY a.career_id, a.name, a.year ORDER BY a.year ASC, a.name ASC", nativeQuery = true)
